...
kappa = 2;
[outqRL] = pulse_current_voltage_RL(outq.u, outq.alpha(2:end-1), 0, 1000, kappa, -1);
...
plot(outqRL.alpha_val, outqRL.I_val, 'LineWidth',3, 'Color', c(3, :))
ylabel('Signal', 'Interpreter', 'latex', 'FontSize',14);
xlabel('$\theta$', 'Interpreter', 'latex', 'FontSize',14);
% ylim([-mi, mi])
legend({'$u(\theta)$', '$I_0(\theta)$', '$I(\theta)$'}, 'interpreter', 'latex', 'location', 'northeast', 'FontSize', 12)
xlim([0, 2*pi])
box off
...
stem(in, log10(nbLk.^2), 'filled', 'Linewidth', 3, 'Color', c(3, :));
...
legend({'$\log(b_\ell^2/\ell^2)$', '$\log(b_\ell^2 (1/(1+(R\ell/L))^2)$', '$\log(b_\ell^2)$',}, 'interpreter', 'latex', 'location', 'southwest', 'FontSize', 12)
xlim([0, nmax])
