...
opts.harmonics.bound_sin = [modulation, modulation; 3*modulation/2, 3*modulation/2];
...
nexttile
hold on
plot(th, modulation*(sin(th) + 3*sin(th*3)/2), 'k', 'linewidth', 3);
...
nexttile
hold on
plot(th, -modulation*(cos(th)+0.5*cos(3*th)), 'k', 'linewidth', 3);
...
plot(th, xi_query+modulation*(cos(th) + 0.5*cos(3*th)), 'linewidth', 3, 'color', cc(3, :));
plot([0, 2*pi], [0, 0], ':k')
xlim([0, 2*pi])
