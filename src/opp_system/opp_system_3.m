classdef opp_system_3 < opp_system_interface
    ...
    methods
       ...
       function return_con = con_return(obj, d)
        ...
        function [su, sd] = sel_jump_monom(obj, d, ind)
            %used for alignment

            %get moments for all jump measures
            sel_orig = obj.jumps.sel_monom_jump(d, ind);

            sel_jump = sel_orig(:, 1);
            for p = 2:size(sel_orig, 2)
                sel_jump = madd_cell_mom(sel_jump, sel_orig(:, p), 1);
            end
            N = length(obj.opts.L_single);
            su = cell(N-1, 1);            
            sd = cell(N-1, 1);     
            ...
            for v = 1:NN
                ...
                if src_i < dst_i
                    %jump up
                    ii = dst_i - 1;
                    su{ii} = su{ii} + sel_jump{v};                    
                elseif src_i > dst_i
                    %jump down
                    ii = dst_i;
                    sd{ii} = sd{ii} + sel_jump{v};
                    
                else
                    %do nothing (party!), take care of this in the rotation
                    %constraints
                end

                % for p = 1:size(sel_orig, 2)
                    % su{ii, ji} = su{ii, ji} + sel_orig{v};
                % end
            end
        end
