    ...
    methods
        ...
        function [vars] = create_vars(obj, opts)
            ...
            if opts.hard_stage_costs
                % explicitly include the harmonics constraints as states

                %TODO: incorporate power losses here.
                [n, bounds, types] = harm_screen(opts);

                mpol(['stage', opts.name], length(n), 1);
                stage = eval(['stage', opts.name]);
                x = [x; stage];
