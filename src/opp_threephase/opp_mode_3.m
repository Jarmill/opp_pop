classdef opp_mode_3 < opp_mode_interface
    ...
    methods
        ...
        function lsupp_base = refine_support(obj, opts, m,  lsupp_ref)
            ...
            Xstop = [stop_pt; lsupp_base.X(2:end)];
            Xstart = [start_pt; lsupp_base.X(2:end)];
        ...
        function mom_3 = get_I_marginals(obj, d)
            ...
            %return the mass of the initial measure in this mode
            va = mmon(obj.vars.x([1, 2, 3]), d);
            vb = mmon(obj.vars.x([1, 2, 4]), d);
            vc = mmon(obj.vars.x([1, 2, 5]), d);

            if obj.Symmetry            
                va = obj.symmetry_eval_current(va, obj.vars.x(3));
                vb = obj.symmetry_eval_current(vb, obj.vars.x(4));
                vc = obj.symmetry_eval_current(vc, obj.vars.x(5));
            end
            ...
            mom_3_cell = obj.mom_sub(obj.vars.x, v3);
            mom_3 = 0;
            for e = 1:N
                mom_3 = mom_3 + mom_3_cell{e};
            end
        ...
        function w_sym = symmetry_eval_current(obj, w_in, vars_trig_I)
            ...
            %NOT Quarter-Wave w(c, s, I) + w(-c, s, I) + w(-c, -s, -I) - w(c, -s, -I)
            ...
            if obj.Symmetry==0
                ...
                %quarter-wave doesn't work naturally in three-phase
                %setting, needs to be hacked.
                % else
                %     Rp_pos = diag([1, -1, 1]);
                %     Rp_neg = diag([-1, 1, -1]);
