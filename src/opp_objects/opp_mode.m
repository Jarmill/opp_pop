classdef opp_mode
    ...
    methods
        function obj = opp_mode(m, vars, opts)
            ...
            X_partition = support_partition(partition, vars)
            
            for n = 1:N
                for j = 1:P
                    curr_level = obj.create_level_loc(n, j, vars, opts);
                end
            end

            for n=1:N
                for j=1:P-1
                    curr_transition = obj.create_transition_loc(n, j, vars, opts);
                end
            end
        end
        ...
        function level_out = create_level_loc(n, j, vars, opts)
            ...
            f_phi = vars.x(3);
            f_load = load_dynamics(vars, opts, n);
        ...
        function f_load = load_dynamics(vars, opts, n)
